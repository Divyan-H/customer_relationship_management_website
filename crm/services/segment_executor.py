...
def _execute_via_postgrest(spec: dict, org_id: str | None = None) -> tuple[int, list[str]]:
    ...
    query = supabase.table("customers").select(
        "id, first_name, last_name, score:customer_scores!inner(*)",
        count="exact",
    )
    ...
    while True:
        res = (
            _org(
                ...
                .select("id, first_name, last_name, city, channel_pref, score:customer_scores!inner(*)"),
                org_id,
            )
...
def _get_ids_postgrest(spec: dict, org_id: str | None = None) -> list[str]:
    ...
    while True:
        query = supabase.table("customers").select(
            "id, score:customer_scores!inner(customer_id)",
        )
        query = _org(query, org_id)
        for cond in spec.get("conditions", []):
            field = cond["field"]
